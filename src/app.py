...
# Safe Groq client retriever (reads Streamlit Secrets first)
def get_groq_client():
    groq_key = None
    try:
        groq_key = st.secrets.get("GROQ_API_KEY")
...
def generate_rag_response(collection, embedder, ranker, groq_client, user_query: str):
    ...
    if not groq_client:
        answer = "⚠️ **API Key Missing**: `GROQ_API_KEY` was not found in Streamlit Secrets. Please verify your settings."
        latency = round(time.time() - start_time, 2)
        log_id = log_interaction(user_query, user_query, answer, latency)
        return user_query, answer, [], latency, log_id
